...
class TransactionController
{
  ...
  public function allTransactions(Request $request, Response $response, $args){
    ...
    $result = $mysqli_connection->query("SELECT * FROM transazioni WHERE account_id = $accountId");
  ...
  public function transaction(Request $request, Response $response, $args){
    ...
    $mysqli_connection = new MySQLi('my_mariadb', 'root', 'ciccio', 'scuola');
    $result = $mysqli_connection->query("SELECT * FROM transazioni WHERE account_id = $accountId AND id = $transactionId");
    $results = $result->fetch_all(MYSQLI_ASSOC);
    if (count($results) === 0) {
  ...
  //POST /accounts/account:id/deposits -- register a deposit action on a specified account
  public function deposit(Request $request, Response $response, $args){
    ...
    $mysqli_connection = new MySQLi('my_mariadb', 'root', 'ciccio', 'scuola');
    $mysqli_connection->query("INSERT INTO transazioni (account_id, amount, description) VALUES ($accountId, $amount, '$description')");
    $response->getBody()->write(json_encode(['message' => 'deposit registered successfully']));
    return $response->withHeader("Content-type", "application/json")->withStatus(201);
  }

  //POST /accounts/account:id/withdrawals -- register a withdrawal action on a specified account
  ...
  public function editDesc(Request $request, Response $response, $args){
    ...
    $description = $data['description'];
    $mysqli_connection = new MySQLi('my_mariadb', 'root', 'ciccio', 'scuola');
    $mysqli_connection->query("UPDATE transazioni SET description = '$description' WHERE account_id = $accountId AND id = $transactionId");
  ...
  public function delete(Request $request, Response $response, $args){
    ...
    $result = $mysqli_connection->query("SELECT id FROM transazioni WHERE account_id = $accountId ORDER BY id DESC LIMIT 1");
    ...
    $mysqli_connection->query("DELETE FROM transazioni WHERE account_id = $accountId AND id = $transactionId");
  ...
  public function getBalance(Request $request, Response $response, $args){
    ...
    $accountId = $args['account'];
    $mysqli_connection = new MySQLi('my_mariadb', 'root', 'ciccio', 'scuola');
    $result = $mysqli_connection->query("SELECT SUM(amount) as balance FROM transazioni WHERE account_id = $accountId");
